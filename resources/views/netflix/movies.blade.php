<x-layouts.netflix title="HabeshaFlix - Movies">
    ...
    <main class="nf-content" style="{{ $isFiltered ? 'padding-top: 2rem;' : '' }}">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 2rem; flex-wrap: wrap; gap: 1rem;">
            <div style="display: flex; align-items: center; gap: 1.5rem;">
                <h1 style="margin: 0; font-size: 2rem;">Movies</h1>
                
                <form action="{{ route('movies') }}" method="GET" style="display: flex; gap: 0.5rem; align-items: center;">
                    @if(request('sort')) <input type="hidden" name="sort" value="{{ request('sort') }}"> @endif
                    
                    <select name="genre" onchange="this.form.submit()" style="background: #000; color: #fff; border: 1px solid #666; padding: 0.3rem 0.5rem; font-size: 0.9rem; border-radius: 2px; cursor: pointer;">
                    ...
                    <select name="year" onchange="this.form.submit()" style="background: #000; color: #fff; border: 1px solid #666; padding: 0.3rem 0.5rem; font-size: 0.9rem; border-radius: 2px; cursor: pointer;">
                    ...
                    @if(request('genre') || request('year') || request('sort'))
                        <a href="{{ route('movies') }}" style="font-size: 0.8rem; color: var(--muted); text-decoration: underline;">Clear all</a>
                    @endif
                </form>
            </div>

            <div style="display: flex; align-items: center; gap: 0.5rem;">
                <span style="font-size: 0.85rem; color: #999;">Sort by:</span>
                <form action="{{ route('movies') }}" method="GET" style="display: flex; gap: 0.5rem;">
                    @if(request('genre')) <input type="hidden" name="genre" value="{{ request('genre') }}"> @endif
                    @if(request('year')) <input type="hidden" name="year" value="{{ request('year') }}"> @endif
                    <select name="sort" onchange="this.form.submit()" style="background: transparent; color: #fff; border: none; font-size: 0.9rem; font-weight: 600; cursor: pointer; outline: none;">
                        <option value="latest" {{ request('sort') == 'latest' ? 'selected' : '' }} style="background: #141414;">Latest</option>
                        <option value="oldest" {{ request('sort') == 'oldest' ? 'selected' : '' }} style="background: #141414;">Oldest</option>
                        <option value="title" {{ request('sort') == 'title' ? 'selected' : '' }} style="background: #141414;">A-Z</option>
                        <option value="year" {{ request('sort') == 'year' ? 'selected' : '' }} style="background: #141414;">Year</option>
                    </select>
                </form>
            </div>
        </div>
        ...
        @if($isFiltered)
            @if($movies->isEmpty())
                <div style="text-align: center; padding: 5rem 0; color: var(--muted);">
                    ...
                    <a href="{{ route('movies') }}" class="nf-btn nf-btn-muted" style="margin-top: 1rem; display: inline-block;">Browse All Movies</a>
                </div>
            @else
